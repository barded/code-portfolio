...
bool Ground::ScreenPtHitsGround(const Matrix4 &view_matrix,
 ...
 */
float hfunc(const Vector3 projection_plane_normal,
  const std::vector < Point3 > &silhouette_curve,
    const Point3 &closest_pt_in_plane) {
...
void Ground::ReshapeGround(const Matrix4 &view_matrix,
  const Matrix4 &proj_matrix,
  ...
  if (!stroke2d.empty()) {
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &first);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), &last);
  }

  Vector3 fl = first - last;
  fl.Normalize();

  Vector3 n = fl.Cross(Vector3(0, 1, 0));
  if (n.Dot(look) > 0) {
    n = -n;
  }
  ...
  std::vector < Point3 > planev;
  for (int i = 1; i < stroke2d.size() - 1; i++) {
    Point3 pplane;
    Point3 pt3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[i]);
    Ray ray(eye, (pt3d - eye).ToUnit());
    float i_time;
    ray.IntersectPlane(first, n, &i_time, &pplane);
    planev.push_back(pplane);
  }
  ...
  for (int i = 0; i < ground_mesh_.num_vertices(); i++) {
    ...
    Point3 newp = P;
    float h = hfunc(n, planev, P.ClosestPointOnPlane(first, n));
    if (h != 0) {
      float d = P.DistanceToPlane(first, n);
      float w = fmax(0, 1 - (d / 5.0) * (d / 5.0));
      newp[1] = (1 - w) * P.y() + w * h;
    }

    new_verts.push_back(newp);
  }
...
void Ground::Draw(const Matrix4 &view_matrix,
  const Matrix4 &proj_matrix,
