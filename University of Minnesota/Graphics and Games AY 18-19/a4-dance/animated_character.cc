...
void AnimatedCharacter::Draw(const Matrix4 &model_matrix,
...
void AnimatedCharacter::DrawBoneRecursive(const std::string &bone_name,
  ...
  */
  ...
  if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
    Matrix4 t = Matrix4::Scale(Vector3(0.10, 0.10, 0.10));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);
  }
  if (bone_name == "lfemur" || bone_name == "rfemur") {
    quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, black, o, o + skeleton_.BoneDirectionAndLength(bone_name), .01);
    Matrix4 t = Matrix4::Scale(Vector3(0.07, 0.07, 0.07));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);
  }
  ...
  if (bone_name == "lowerback") {
    Matrix4 t = Matrix4::Translation(Vector3(0, 0, -.1)) * Matrix4::RotationX(-M_PI / 4.0) * Matrix4::Scale(Vector3(0.12, 0.12, 0.3));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);
  }
  if (bone_name == "upperback") {}
  if (bone_name == "thorax") {
    Matrix4 t = Matrix4::Scale(Vector3(0.08, 0.08, 0.08));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);
  }
  if (bone_name == "lowerneck" || bone_name == "upperneck") {
    Matrix4 t = Matrix4::Scale(Vector3(0.05, 0.05, 0.05));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);
  }
  if (bone_name == "head") {
    Matrix4 t = Matrix4::RotationX(M_PI / 4.0) * Matrix4::Scale(Vector3(0.08, 0.05, 0.15));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);

    Matrix4 leye = Matrix4::Translation(Vector3(.05, .07, -.015)) * Matrix4::Scale(Vector3(0.02, 0.02, 0.02));
    quick_shapes_.DrawSphere(ctm * leye, view_matrix, proj_matrix, blue);

    Matrix4 reye = Matrix4::Translation(Vector3(-.05, .07, -.015)) * Matrix4::Scale(Vector3(0.02, 0.02, 0.02));
    quick_shapes_.DrawSphere(ctm * reye, view_matrix, proj_matrix, blue);

    Matrix4 mouth = Matrix4::Translation(Vector3(0, -.02, .08)) * Matrix4::Scale(Vector3(0.03, 0.02, 0.01));
    quick_shapes_.DrawSphere(ctm * mouth, view_matrix, proj_matrix, black);

    Matrix4 lanten = Matrix4::Translation(Vector3(0.05, 0.15, -.14)) * Matrix4::RotationX(-M_PI / 4.0) * Matrix4::RotationZ(-M_PI / 6.0) * Matrix4::Scale(Vector3(0.01, 0.08, 0.01));
    quick_shapes_.DrawCone(ctm * lanten, view_matrix, proj_matrix, black);

    Matrix4 ranten = Matrix4::Translation(Vector3(-0.05, 0.15, -.14)) * Matrix4::RotationX(-M_PI / 4.0) * Matrix4::RotationZ(M_PI / 6.0) * Matrix4::Scale(Vector3(0.01, 0.08, 0.01));
    quick_shapes_.DrawCone(ctm * ranten, view_matrix, proj_matrix, black);
  }
  ...
  if (bone_name == "lhand" || bone_name == "rhand" || bone_name == "lthumb" || bone_name == "rthumb" || bone_name == "rfingers" || bone_name == "lfingers") {
    quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, black, o, o + skeleton_.BoneDirectionAndLength(bone_name), .01);
    Matrix4 t = Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
    quick_shapes_.DrawSphere(ctm * t, view_matrix, proj_matrix, red);
