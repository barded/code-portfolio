...
void ArtRenderApp::OnLeftMouseDrag(const Point2 &pos,
  const Vector2 &delta) {
  Point2 normalizedMousePos = PixelsToNormalizedDeviceCoords(pos);
...
void ArtRenderApp::DrawUsingOpenGL() {
  ...
  // Lighting parameters
  static
  const Color Ia(0.3, 0.3, 0.3, 1);
  static
  const Color Id(0.7, 0.7, 0.7, 1);
  static
  const Color Is(1.0, 1.0, 1.0, 1);

  // Material parameters
  static
  const Color ka(1.0, 0.4, 0.4, 1);
  static
  const Color kd(1.0, 0.4, 0.4, 1);
  static
  const Color ks(0.6, 0.6, 0.6, 1);
  static
  const float s = 50;
