...
class YearlyTestData {
 ...
 public YearlyTestData(int year) {
  this.year = year;
  this.level = 0;
  this.num_student = 0;
  this.hours = 0;
...
class ESLTestData {
 private HashMap < Integer, YearlyTestData > dataByYear;

 // Constructor just initializes internal variables
 public ESLTestData() {
  dataByYear = new HashMap < Integer, YearlyTestData > ();
 }
 ...
 public void loadFromFile(String fileName) {
  ...
  for (TableRow rawRow: rawData.rows()) {
   ...
   if (!dataByYear.containsKey(year)) {
    YearlyTestData yd = new YearlyTestData(year);
 ...
 public YearlyTestData getYearlyTestData(int year) {
  if (dataByYear.containsKey(year)) {
