...
class PixelBuffer {
 public:
  ...
  /** @brief Returns a pointer to the raw pixel data array stored in row-major
    order as RGBA floats. */
  inline float const *data() const { return &pixels_[0]; }

  /** @brief Resizes the buffer, cropping the image and/or creating new pixels
    as needed. New pixels will use the background color. */
  void Resize(int new_width, int new_height);

  /** @brief Saves to a PNG file, the filename should include
    a .png extension. */
  void SaveToFile(const std::string &filename);

  /** @brief Loads from a PNG file.  Will resize the buffer as needed. */
