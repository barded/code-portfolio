...
class ImageEditor {
 public:
  /** @brief The ImageEditor requires a current pixel buffer, so this is the
   preferred constructor. */
  explicit ImageEditor(PixelBuffer *buffer);

  /** @brief Creates an editor with a NULL pixel buffer.  You will need to set
   the pixel buffer before performing any editing operations. */
  ImageEditor();
  ...
  /** @brief Returns a pointer to the Tool member variable with the input
    name. */
  Tool *GetToolByName(const std::string &name);

  /** @brief Call this from the controller to being a new stroke at pixel (x,y)
   using the tool named tool_name and the specified color and radius.  Since it
  ...
  /** @brief Call this from the controller to apply the blur filter to the
   current pixel buffer using the current blur filter state. */
  ...
  /** @brief Call this from the controller to apply the sharpen filter to the
   current pixel buffer using the current sharpen filter state. */
  void ApplySharpenFilter(float radius);

  /** @brief Call this from the controller to apply the edge detect filter to
   the current pixel buffer using the current edge detect filter state. */
  void ApplyEdgeDetectFilter();

  /** @brief Call this from the controller to apply the threshold filter to the
   current pixel buffer using the current threshold filter state. */
  void ApplyThresholdFilter(float cutoff_value);

  /** @brief Call this from the controller to apply the saturate filter to the
   current pixel buffer using the current saturate filter state. */
  void ApplySaturateFilter(float scale_factor);

  /** @brief Call this from the controller to apply the channels filter to the
   current pixel buffer using the current channels filter state. */
  void ApplyChannelsFilter(float red_scale, float green_scale,
                           float blue_scale);

  /** @brief Call this from the controller to apply the quantize filter to the
  ...
  /** @brief True if the log of undone commands is not empty, i.e., it is
