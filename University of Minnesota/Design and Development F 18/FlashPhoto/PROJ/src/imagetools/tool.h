...
class Tool {
 public:
  ...
  /** @brief Subclasses must fill in this factory method to define the mask for
   the specific tool they implement. */
  virtual FloatMatrix *CreateMask(float radius) = 0;

  /** @brief The controller (typically the GUI class) must call this method each
   time the user starts a new stroke.  The buffer, paint_color, and radius are
  ...
  /** @brief The controller should call this function after calling
    StartStroke() to add additional line segments to the stroke each time the
    mouse moves. In other words, each call to StartStroke() should by followed
    by multiple calls to AddToStroke().  Keep calling AddToStroke() until the
    user lifts up the mouse button. */
  virtual void AddToStroke(int x, int y);

  /** @brief The controller must call this method to complete the stroke when
   the user releases the mouse button. */
  void EndStroke(int x, int y);

  /** @brief This function will stamp the current mask onto the current buffer
   exactly one time at a location centered at (x,y) within the buffer. */
  void StampMaskOntoBuffer(int x, int y);

  /** @brief This function returns the current paint color used with the tool.
   In most cases, this is the same as the paint_color passed in to the
  ...
  /** @brief This function combines (i.e., blends) a paint_color with an
   underlying canvas_color given a mask_intensity.  The default algorithm uses
    a standard blending function:
  ...
  /** @brief Most tools only apply "paint" when they are moved on the canvas,
   but some, like a spray can continue applying more paint over time even if
