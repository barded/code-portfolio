...
void PixelBuffer::SaveToFile(const std::string &filename) {
  ...
  for (int y = 0; y < height(); y++) {
    for (int x = 0; x < width(); x++) {
      int index = 4 * (x + width_ * y);  // x + width*(height-(y+1));
      image->SetFloatValue(x, y, 0, pixels_[index + 0]);
      image->SetFloatValue(x, y, 1, pixels_[index + 1]);
      image->SetFloatValue(x, y, 2, pixels_[index + 2]);
      image->SetFloatValue(x, y, 3, pixels_[index + 3]);
    }
  }
